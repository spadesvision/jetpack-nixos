...
let
  ...
  mkFlashScriptAuto = if builtins.length cfg.firmware.variants == 1 then
    mkFlashCmdScript
  else
  ...
  flashScript = writeShellApplication {
    derivationArgs.pname = "flash-${hostName}";
    name = "flash-${hostName}";
...
in { inherit flashScript initrdFlashScript fuseScript rcmBoot; }
