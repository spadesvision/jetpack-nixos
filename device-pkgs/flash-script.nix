{ lib, preFlashCommands ? "", flashCommands ? "", postFlashCommands ? ""
, flashArgs ? [ ], partitionTemplate ? null, socType ? null,
...
additionalDtbOverlays ? [ ], flash-tools, }:
(''
  set -euo pipefail
  ...
  export ADDITIONAL_DTB_OVERLAY=''${ADDITIONAL_DTB_OVERLAY:+$ADDITIONAL_DTB_OVERLAY,}${
    lib.concatStringsSep "," additionalDtbOverlays
  }

  ${lib.optionalString (partitionTemplate != null)
...
'' else ''
  ./flash.sh ${
    lib.optionalString (partitionTemplate != null) "-c flash.xml"
  } "$@" ${builtins.toString flashArgs}
'') + ''
  ${postFlashCommands}
'')
